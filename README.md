...
- 按游戏装备栏布局（头盔/胸甲/护腿/靴子/主手 + 双护符/项链/戒指/图腾/副手共 11 槽）自由搭配装备，实时计算**完整属性面板**
- 计算公式逐条对齐游戏客户端源码的 Calc 系列：`生命 = round((基础20+flat)×(1+pct/100))`、`物伤 = round((基础5~10+装备)×(1+pct/100))`、`攻频 = 1.2×(1+min(攻速,30)/30)+急速×0.35` 等
- **额外加成**支持手动填入护符镌刻词条等随机属性（护符/精工/大师词条为随机生成，无法从数据接口获取）
- 装备 Buff 自动聚合（套装同名不叠加，取最高等级），并在面板中展示效果说明
- 每项属性附带**贡献明细**（装备 +X ｜ 额外 +Y），攻速超上限、唯一装备重复佩戴会给出提示
- **输出估算**：单次期望伤害与期望 DPS（含暴击期望、力量Buff、最终伤害放大、远程×连射）
- 配装方案可保存/载入（localStorage 持久化），方便横向对比多套搭配
- 局限：武器/护甲库收藏属性为个人存档数据，暂无法纳入计算
